...
void usingRealloc(size_t length)
{
    int *p = (int *) calloc(length,sizeof(int));
    ...
    if(p != NULL)
    {
        ...
        /* Reallocate memory to hold double the elements */
        size_t new_length = length * 2;
        int *temp = (int *) realloc(p, new_length * sizeof(int));

        if(temp == NULL)
        {
            cout << "The realloc function failed to allocate memory" << endl;
        }
        else
        {
            p = temp;
