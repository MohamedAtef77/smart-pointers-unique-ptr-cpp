...
int main(void)
{
    ...
    /* auto_ptr owns a heap object and destroys it automatically on scope exit */
    demo_basic_ownership();

    /* release() surrenders raw pointer ownership; caller must delete manually */
    demo_release();

    /* reset() destroys the current object and optionally adopts a new one */
    demo_reset();

    /* copy construction silently transfers ownership, leaving the source null */
    demo_ownership_transfer_via_copy();

    /* copy assignment destroys the target's object then steals the source's */
    demo_ownership_transfer_via_assignment();

    /* container copies internally steal ownership, producing null-filled containers */
    demo_container_hazard();
